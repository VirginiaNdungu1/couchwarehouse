...
const fs = require('fs')
let config
// IBM's Databases For PostgreSQL needs a CA
...
const insertBulk = async (opts, createSQL, dbName, theSchema, batch) => {
  ...
  batch.forEach(async b => {
    await insertDoc(b, opts, dbName, theSchema)
  })
...
const insertDoc = async (b, opts, dbName, theSchema) => {
  // ignore design docs
  if (!b.id.match(/^_design/) && b.doc) {
    ...
    console.log(b)
    // if answersheet also send answers
    if (docType === 'answersheet') {

      const answers = JSON.parse(b.doc.answers);
      answersArray = [];
      for (const group in answers) {
        for (const field in answers[group]) {
          console.log(`field: ${field}: ${answers[group][field]}`);
          
          const value = answers[group][field];
          const fieldId = b.doc._id + '_' + group + '_' + field;
          const answer = {
            "_id": fieldId,
            "doc": {
              "_id": fieldId,
      ...
      await Promise.all(answersArray.map(async answer => {
        const result = await insertChild(answer, opts, dbName, 'answer', theSchema['answer']);
        return result;
      }));
    }
...
const insertChild = async (b, opts, dbName, docType, childSchema) => {

  // get list of fields
  const fields = Object.keys(childSchema)
  const replacements = []
  const pairs = []
  let j = 1
  fields.forEach((f) => {
